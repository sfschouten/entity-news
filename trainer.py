...
def default_output_to_logits(outputs, ignore_keys=()):
...
class Trainer(transformers.Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.output_to_logits_fn = default_output_to_logits
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    if isinstance(raw_outputs, dict):
                        ...
                        logits_mb = self.output_to_logits_fn(
                            raw_outputs,
                            ignore_keys=ignore_keys + ["loss"]
                        )
                    else:
                    ...
                    loss = None
                    if isinstance(raw_outputs, dict):
                        logits_mb = self.output_to_logits_fn(raw_outputs, ignore_keys=ignore_keys)
                    else:
                ...
                if has_labels:
                    ...
                    if isinstance(outputs, dict):
                        logits = self.output_to_logits_fn(outputs, ignore_keys=ignore_keys + ["loss"])
                    else:
                        logits = outputs[1:]
                else:
                    loss = None
                    ...
                    if isinstance(outputs, dict):
                        logits = self.output_to_logits_fn(outputs, ignore_keys=ignore_keys)
                    else:
                        logits = outputs
